...
fn attr_value(open_tag: &str, orig_open_tag: &str, name: &str) -> Option<String> {
    ...
    while let Some(rel) = rest.find(name) {
        ...
        // `class`).
        if rel > 0 {
            let prev = rest[..rel].chars().last();
            if prev.is_some_and(|p| p.is_ascii_alphanumeric() || p == '-' || p == '_') {
                rest = after;
                offset = abs + name.len();
                continue;
            }
...
fn find_matching_close(lower_html: &str, tag: &str, content_start: usize) -> Option<usize> {
    ...
    while offset < lower_html.len() {
        ...
        let take_open = match (next_open, next_close) {
            (Some(no), Some(nc)) => (no < nc).then_some(no),
            (Some(no), None) => Some(no),
            (None, _) => None,
        };
        if let Some(no) = take_open {
            depth += 1;
            offset += no + 1;
        } else {
            let nc = next_close?;
